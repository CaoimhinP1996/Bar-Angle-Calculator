...
def zeropointcopy(redclump): #calculate zero point for each star
    df = redclump["fit cut"]
...
def zpcut(x,E,F,rczp,zpNrc,szprc): # function for fitting parallax w/ zero point
    return E*np.exp(F*(x-rczp)) + \
           zpNrc/(np.sqrt(2*np.pi*szprc**2))*np.exp(-0.5*(x-rczp)**2/szprc**2)

def pcut(x,H,I,rcp,pNrc,sprc): # function for fitting parallax w/ zero point
...
def finalfit(redclump, zero_point): # find peak parallax along sightline from fitting distribution of parallax before 
    ...
    zphist,zpbe = np.histogram(zp["parallax"], bins=40)
    zpbc = 0.5*(zpbe[0:-1]+zpbe[1:])
    zpguess = [(zphist[0]+zphist[-1])/2,0.5,0.1,(zphist[0]+zphist[-1])/2,0.1]
    zppar,zpcov = spopt.curve_fit(zpcut,zpbc,zphist,p0=zpguess,sigma=np.sqrt(zphist))

    phist,pbe = np.histogram(prezp["parallax"], bins=40)
    pbc = 0.5*(pbe[0:-1]+pbe[1:])
    pguess = [(phist[0]+phist[-1])/2,0.5,0.1,(phist[0]+phist[-1])/2,0.1]
    ppar,pcov = spopt.curve_fit(pcut,pbc,phist,p0=pguess,sigma=np.sqrt(phist))

    E,F,rczp,zpNrc,szprc = zppar
    H,I,rcp,pNrc,sprc = ppar

    fitzp = (zp.loc[(zp["parallax"]> rczp - szprc) & (zp["parallax"]< rczp + szprc)])

    fitprezp = (prezp.loc[(prezp["parallax"]> rcp - sprc) & (prezp["parallax"]< rcp + sprc)])

    output = {"zphist": zphist,
              "zpbc": zpbc,
              "zpbe": zpbe,
              "zpguess": zpguess,
              "zppar": zppar,
              ...
              "zp parallax peak": rczp,
              "prezp parallax peak": rcp,
              "zp parallax error": szprc,
...
def finalcut(redclump, zero_point): # sigma clip based on parallax to further isolate the bar red clump and find mean parallax
    ...
    zp = zp[~clipped.mask]
    zpparerr = 1/zp["parallax error"]**2
    zpparerrmean = np.sqrt(1/np.sum(zpparerr))
    zpparmean = np.sum(zpparerr*zp["parallax"]/np.sum(zpparerr))
    prezpparerr = 1/prezp["parallax_error"]**2
    prezpparerrmean = np.sqrt(1/np.sum(prezpparerr))
    prezpparmean = np.sum(prezpparerr*prezp["parallax"]/np.sum(prezpparerr))
    output = {"zp parallax mean error": zpparerrmean,
              "zp parallax mean": zpparmean,
              "post-zp dataframe": zp,
              "pre-zp parallax mean error": prezpparerrmean,
              "pre-zp parallax mean": prezpparmean,
